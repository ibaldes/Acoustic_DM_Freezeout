...
delta = np.linspace(0, np.pi, 76)

norm = bmannDMext(1000, sigmadm, 0)
...
# parallel implementation - I define some functions to be used by pool. Alternative to passing arguments to starpool.

def func1(delta):
	return(bmannDMDText(1000, sigmadm, 0, 0.2, delta, 0.3)/norm)
	
def func2(delta):
	return(bmannDMDText(1000, sigmadm, 0, 0.2, delta, 1.0)/norm)
	
def func3(delta):
	return(bmannDMDText(1000, sigmadm, 0, 0.2, delta, 10)/norm)
...
with mp.Pool() as pool:
    results1 = pool.map(func1, delta)
    
print("results 1/3 done")
#print(results1)
    
with mp.Pool() as pool:
    results2 = pool.map(func2, delta)

print("results 2/3 done")
#print(results2)
    
with mp.Pool() as pool:
    results3 = pool.map(func3, delta)

print("results 3/3 done")
#print(results3)    

from scipy.interpolate import make_interp_spline

results1int = make_interp_spline(delta, results1)
results2int = make_interp_spline(delta, results2)
results3int = make_interp_spline(delta, results3)
	
plt.title("s-wave, $|R_i|$=0.2,  $m_{\mathrm{DM}} = 1$ TeV")
plt.plot(delta, results1int(delta), 'blue', label=r"$\bar{x}_H = 0.3$")
plt.plot(delta, results2int(delta), 'purple', label=r"$\bar{x}_H = 1$")
plt.plot(delta, results3int(delta), 'red', label=r"$\bar{x}_H = 10$")
plt.plot(delta, yout4, 'gray')
plt.legend(loc="lower right")
ax = plt.gca()
ax.set_xlim([0, np.pi])
ax.set_ylim([0.9, 1.15])
plt.xlabel(r"$\delta$")
plt.ylabel(r"$Y(\delta)/Y(\delta=\pi/2)$")
plt.savefig("Fig3.jpg")
plt.clf()

print("Finished making Fig. 3")
