...
Rirange = np.linspace(0,0.2,11)

normSE = sigmareqext(1000,-1/2)
normSWAVE = sigmareqext(1000,0)
normPWAVE = sigmareqext(1000,1)
normDWAVE = sigmareqext(1000,2)

sigmaratreqSEvals = []
sigmaratreqSWAVEvals = []
sigmaratreqPWAVEvals = []
sigmaratreqDWAVEvals = []

for Ri in Rirange:
	print("Ri = ", Ri)
	sigmaratreqSEvals.append(sigmareqDT(1000, -1/2, Ri, 1)/normSE)	
	sigmaratreqSWAVEvals.append(sigmareqDT(1000, 0, Ri, 1)/normSWAVE)
	sigmaratreqPWAVEvals.append(sigmareqDT(1000, 1, Ri, 1)/normPWAVE)
	sigmaratreqDWAVEvals.append(sigmareqDT(1000, 2, Ri, 1)/normDWAVE)
	

from scipy.interpolate import make_interp_spline

sigmaratreqSEint = make_interp_spline(Rirange, sigmaratreqSEvals)
sigmaratreqSWAVEint = make_interp_spline(Rirange, sigmaratreqSWAVEvals)
sigmaratreqPWAVEint = make_interp_spline(Rirange, sigmaratreqPWAVEvals)
sigmaratreqDWAVEint = make_interp_spline(Rirange, sigmaratreqDWAVEvals)
	
plt.title(r"$m_{\mathrm{DM}} = 1$ TeV, $\bar{x}_H = 1$")
plt.plot(Rirange, sigmaratreqSEint(Rirange), 'cyan', label="n=-1/2")
plt.plot(Rirange, sigmaratreqSWAVEint(Rirange), 'blue', label="n=0")
plt.plot(Rirange, sigmaratreqPWAVEint(Rirange), 'purple', label="n=1")
plt.plot(Rirange, sigmaratreqDWAVEint(Rirange), 'red', label="n=2")
